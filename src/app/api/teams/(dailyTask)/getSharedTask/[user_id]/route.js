...
export async function GET(req, { params }) {
    try {
       ...
       console.log(user_id);
        ...
        const share_with = decoded.user._id
        console.log(share_with);
        const tasks = await DailyTask.find({ _id:user_id,share_with:share_with }).sort({ createdAt: -1 })
        console.log(tasks);
        ...
        return NextResponse.json({ message: "Tasks fetched successfully"}, { status: 200 });
